...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const dongParam = values[values.length - 1] || ""; // 항상 마지막 주소 조각(동)을 안전하게 가져옴
  ...
  let title = `${districtName} ${decodedDong} 출장 힐링 마사지 24시 신속방문 제휴안내 | 릴렉스몽`;
  let description = `${districtName} ${decodedDong} 출장 힐링 마사지 실시간 예약. 30분 내 빠른 도착과 100% 현장 후불 정찰제로 편안한 홈타이·스웨디시를 경험하세요.`;
